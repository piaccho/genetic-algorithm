...
from cross_methods import arithmetic_crossover, linear_crossover, alpha_crossover, alpha_beta_crossover, averaging_crossover
from mutation import uniform_mutation, gaussian_mutation
...
class Population:
    def __init__(self, size, chromosome_length, fitness_function, num_variables, begin_range, end_range):
        self.individuals = [
            Individual(
                chromosome=RealChromosome(chromosome_length)
            ) for _ in range(size)
        ]
        ...
        for _ in range(len(self.individuals) // 2):
            if np.random.rand() < cross_probability:
                ...
                if method == 'arithmetic':
                    child1, child2 = arithmetic_crossover(parent1.chromosome, parent2.chromosome, alpha=np.random.uniform(0, 1))
                elif method == 'linear':
                    child1, child2 = linear_crossover(parent1.chromosome, parent2.chromosome, self.fitness_function)
                elif method == 'alpha':
                    child1, child2 = alpha_crossover(parent1.chromosome, parent2.chromosome)
                elif method == 'alpha_beta':
                    child1, child2 = alpha_beta_crossover(parent1.chromosome, parent2.chromosome)
                elif method == 'averaging':
                    child1, child2 = averaging_crossover(parent1.chromosome, parent2.chromosome)
    ...
    def mutate(self, method, mutation_probability):
        for individual in self.individuals:
            if np.random.rand() < mutation_probability:
                if method == 'uniform':
                    uniform_mutation(individual.chromosome)
                elif method == 'gaussian':
                    gaussian_mutation(individual.chromosome)
